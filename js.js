...
const map = {
  ...
  render(snakePointsArray, foodPoint, trapPoint,) {
    ...
    const trapCell = this.cells[`x${trapPoint.x}_y${trapPoint.y}`];

    foodCell.classList.add('food');
    trapCell.classList.add('trap');
    this.usedCells.push(foodCell, trapCell,);
  },
};
...
const game = {
  ...
  getRandomFreeCoordinates() {
    const exclude = [this.food.getCoordinates(), ...this.snake.getBody(), this.trap.getCoordinates()];
    // without ... -  [{}, [{}, {}, {}]] => with ... [{}, {}, {}, {}];
    while (true) {
      const rndPoint = {
  ...
  render() {
    this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.trap.getCoordinates());
  },
  ...
  tickHandler() {
    ...
    if (this.trap.isOnPoint(this.snake.getNextStepHeadPoint())) {
      this.finish(); // handlertrap
