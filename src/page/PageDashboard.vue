<template>
 <div>
  <v-row>
    <v-col>
      <div class="text-h5 text-md-3">My Personal Costs</div>
      <v-dialog
      v-model="dialog"
      width="500">
      ...
      <v-btn color="teal" dark v-on="on">ADD NEW COST<v-icon>mdi-plus</v-icon></v-btn>
        ...
        <v-card>
          <AddPaymentForm @addNewPayment="addNewPaymentData" :category-list="categoryList" :value="123"/>
        </v-card>
      </v-dialog>
    </v-col>
  </v-row>
  <PaymentsDisplay :items="curElements" :fullList="paymentList"/>
  <Pagination
