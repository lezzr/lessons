<template>
  <div>
    <div class="error" :class="{'red': error}" v-show="!!error">{{error}}</div>
    <br>
    <input type="number" v-model.number="operand1" placeholder="operand1" id="op1"/>
    <input type="number" v-model.number="operand2" placeholder="operand2" id="op2"/>
...
export default {
  ...
  methods: {
    ...
    fulldiv() {
     ...
     remLast(){
      const rem = this[this.picked]
       const slc = rem.slice(0, -1)
      console.log(slc)
    }
  },
  computed: {
